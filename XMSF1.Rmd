...
ggplot(self, aes(x = age_cs, y = riskSR, color = as.factor(sex))) + geom_point(alpha=.4)+geom_smooth(method = "lm") +
...
anova(mod2, mod1) #mod2 is better model than mod1
```
...
#48.0% for normal trials & 38.7% for high risk trials
```

Q10. How do you interpret this in words?
```{r q10}
#In high risk conditions, participants were more likely to choose not to take the risk
```
...
```{r q12}
mod4 <- glmer(RiskyChoice ~ trialtype
              + (1 + trialtype | partid) + (1| scenario),
              family='binomial',
              data=combi)
summary(mod4)
```
