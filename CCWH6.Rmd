...
```{r q01}
#Used mixed regression model
#DV is self reported risk attitude
#random effects of participants and sex ##
```
...
```{r q03}
ggplot(self, aes(x=age, y = riskSR, fill=as.factor(sex))) + geom_point() + geom_smooth(method = lm)
```
...
```{r q04}
ggplot(self, aes(x=age, y = riskSR, fill=as.factor(sex))) + geom_point() + geom_smooth(method = lm) + scale_fill_discrete(name = "Sex", labels = c("male", "female"))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#Doesn't seem like there is a correlation
```
...
```{r q09}
combi %>% group_by(trialtype, RiskyChoice) %>% summarise(n=n())

3451/(3742+3451+4419+2789)*100
#23.96%
```
