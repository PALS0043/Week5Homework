...
#the DV is self reported risk attitude 
#Participant could be a random effect 
```
...
```{r q03}
#sex has to be character 
self <- self %>% mutate(sex = dplyr::recode(sex, '1' = 'female', '0' = 'male'))
...
```{r q04}
ggplot(self, aes(x = age, y = riskSR, colour = as.factor(sex))) + geom_point() + geom_smooth(method = 'lm') + labs(colour = "sex")
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#males self-report taking less risks as they grow older, yet females barely see a change in self-reported risk taking as they age
```
...
#lmer requires random effects to be added. Assuming the question is not asking for random effects to be included, I have changed lmer() to lm()
...
```{r q09}
combi %>% group_by(trialtype) %>% summarise(choice = mean(RiskyChoice))

```

Q10. How do you interpret this in words?
```{r q10}
#For the normal trials, 48.0% of people made the risky choice. For the high risk trials, 38.7% of people made the risky choice
```
