...
```{r q01}
#type of model: mixed effects model; DV: self reported risk attitude; random effects: participants
```
...
```{r q03}
ggplot(aes(x = age_cs, y = riskSR, group = as.factor(sex)), data = self)+
  geom_point()+
  theme_bw()+
...
```{r q04}
ggplot(aes(x = age_cs, y = riskSR, color = as.factor(sex), data = self)+
  geom_point()+
  theme_bw()+
  geom_smooth(method = lm)+
  scale_colour_manual(name="sex", labels=c("male","female")))
```

Q5. What does the graph suggest about risk attitudes
```{r q05}
#when the age is larger, the risk attitude is lower, which means older people are less willing to take risks.
```
...
summary(mod2)
#yes, mod2 explains more variance
```
...
3451/(3451+3742) #48% under 'normal'
2789/(2789+4419) #39% under 'risk'
```
...
#people are more likely to choose safely when they are in risk conditions.
```
...
# I sucessfully commit the homework file but I failed to push it to GitHub. When I click the green push button, the git pane shows below: 
